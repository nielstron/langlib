...
- **Automata.** *Deterministic pushdown automata* (DPDA): a pushdown automaton with at
  ...
  `is_DCF_total`.
- **Grammars.** *LR(k) grammars*: context-free grammars whose rightmost handle is uniquely determined by
  the already-read prefix and `k` terminals of lookahead. For every fixed `k > 0`,
