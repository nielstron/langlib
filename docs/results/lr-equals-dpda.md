...
recognized by deterministic pushdown automata (DPDAs).

The canonical `is_LRk` definition quantifies over the repository's unrestricted
(Type-0) grammars. Its `grammar_lrk` restriction combines context-free rule shape
with a handle-uniqueness condition stated directly on unrestricted rules. The
older `CF_grammar` presentation remains available as `is_LRk_via_cfg`, and the
two presentations are proved equivalent.

Consequently, allowing an existential finite lookahead does not enlarge the
class:
...
The direct unrestricted definition and its compatibility bridge are in
[`Langlib.Grammars.LR.Definition`](../../api/Langlib/Grammars/LR/Definition.html)
and
[`Langlib.Grammars.LR.UnrestrictedCharacterization`](../../api/Langlib/Grammars/LR/UnrestrictedCharacterization.html).
