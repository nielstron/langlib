...
def grammar_language (g : grammar T) : Language T := setOf (grammar_generates g)
...
def RG : Set (Language T) := setOf is_RG
...
def Linear : Set (Language T) := setOf is_Linear
...
def CF : Set (Language T) := setOf is_CF
...
def CS : Set (Language T) := setOf is_CS
...
def RE : Set (Language T) := setOf is_RE
...
def DFA.Class : Set (Language T) := setOf is_DFA
...
def NFA.Class : Set (Language T) := setOf is_NFA
...
-- Reserve the two auxiliary names consumed by the source module's public
-- declaration elaboration so the structure-field proofs retain their exact
-- exported names (`_proof_3` and `_proof_4`).
theorem toPDA._proof_1 : True := trivial
theorem toPDA._proof_2 : True := trivial
/-- Eliminator used to turn an optional deterministic transition into a PDA transition set. -/
abbrev toPDA.match_1.{u_1} {Q S : Type}
    (motive : Option (Q × List S) → Sort u_1)
    (x : Option (Q × List S))
    (h_1 : (p : Q × List S) → motive (some p))
    (h_2 : (a : Unit) → motive none) : motive x :=
  Option.casesOn x (h_2 Unit.unit) fun val => h_1 val
theorem toPDA._proof_3 (M : DPDA Q T S) (q : Q) (a : T) (Z : S) :
    Set.Finite (toPDA.match_1 (fun _ => Set (Q × List S))
      (M.transition q a Z) (fun p => ({p} : Set (Q × List S)))
      (fun (_ : Unit) => (∅ : Set (Q × List S)))) := by
  cases M.transition q a Z with
  | none => exact Set.toFinite ∅
  | some p => exact Set.toFinite {p}
theorem toPDA._proof_4 (M : DPDA Q T S) (q : Q) (Z : S) :
    Set.Finite (toPDA.match_1 (fun _ => Set (Q × List S))
      (M.epsilon_transition q Z) (fun p => ({p} : Set (Q × List S)))
      (fun (_ : Unit) => (∅ : Set (Q × List S)))) := by
...
noncomputable def toPDA (M : DPDA Q T S) : PDA Q T S where
  ...
  transition_fun q a Z := toPDA.match_1 (fun _ => Set (Q × List S))
    (M.transition q a Z) (fun p => ({p} : Set (Q × List S)))
    (fun (_ : Unit) => (∅ : Set (Q × List S)))
  transition_fun' q Z := toPDA.match_1 (fun _ => Set (Q × List S))
    (M.epsilon_transition q Z) (fun p => ({p} : Set (Q × List S)))
    (fun (_ : Unit) => (∅ : Set (Q × List S)))
  finite q a Z := toPDA._proof_3 M q a Z
  finite' q Z := toPDA._proof_4 M q Z
...
def DPDA.Class : Set (Language T) := setOf is_DPDA
...
def CF_language (g : CF_grammar T) : Language T := setOf (CF_generates g)
...
def Language (g : IndexedGrammar T) : _root_.Language T := setOf (Generates g)
...
def Indexed : Set (Language T) := setOf is_Indexed
...
def LBA {T : Type} [Fintype T] [DecidableEq T] : Set (Language T) := setOf is_LBA
