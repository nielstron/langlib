...
- `is_CF_of_is_RG` — Every right-regular language is context-free.
- `isRegular_of_is_RG` — Every right-regular grammar language is Mathlib-regular.
- `is_RG_of_isRegular` — Every Mathlib-regular language (over a finite alphabet) is
  generated by a right-regular grammar.
- `is_RG_iff_isRegular` — Equivalence (over a finite alphabet).
- `is_DCFL_of_is_RG` — Every right-regular language is a DCFL.
