...
/-!
# Context-Sensitive Languages = flag-LBA Languages

This file states the Myhill–Landweber–Kuroda theorem in its set-level form for the internal
marker-free **flag** model (`Automata/LinearBounded/FlagModel.lean`),

  `CS = LBA_flag`  (`CS_eq_LBA_flag`),
...
class of languages recognised by a nondeterministic flag LBA (`LBA_flag`). The headline
`CS = LBA` for the canonical *endmarker* model is derived from this in
`Equivalence/EndmarkerToFlag.lean` (`CS_eq_LBA`), via `LBA = LBA_flag`.
...
theorem LBA_flag_subset_CS : (LBA_flag : Set (Language T)) ⊆ CS := by
...
theorem CS_subset_LBA_flag : (CS : Set (Language T)) ⊆ LBA_flag := by
  classical
  rintro L ⟨g, hg, rfl⟩
...
/-- **Context-sensitive languages are exactly the flag-LBA languages.** The canonical-model
version `CS = LBA` is `CS_eq_LBA` in `Equivalence/EndmarkerToFlag.lean`. -/
theorem CS_eq_LBA_flag : (CS : Set (Language T)) = LBA_flag :=
  Set.Subset.antisymm CS_subset_LBA_flag LBA_flag_subset_CS
