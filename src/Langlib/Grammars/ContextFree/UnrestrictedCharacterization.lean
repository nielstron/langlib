...
- `is_CF_via_cfg` — language class induced by `CF_grammar`
- `is_CF_via_cfg_implies_is_CF` — conversion to the default unrestricted characterization
- `is_CF_implies_is_CF_via_cfg` — extraction of a `CF_grammar` witness
...
public lemma CF_language_eq_grammar_language (g : CF_grammar T) :
    CF_language g = grammar_language (grammar_of_cfg g) := by
  unfold CF_language
  unfold grammar_language
  ext1 w
  change CF_generates g w ↔ grammar_generates (grammar_of_cfg g) w
...
public theorem grammar_of_cfg_context_free (g : CF_grammar T) :
    grammar_context_free (grammar_of_cfg g) := by
  intro r hr
  change r ∈ List.map (fun r : g.nt × List (symbol T g.nt) =>
    grule.mk [] r.fst [] r.snd) g.rules at hr
  obtain ⟨r', _, hrr⟩ := List.mem_map.mp hr
  exact ⟨(congrArg grule.input_L hrr).symm, (congrArg grule.input_R hrr).symm⟩

/-- Every language generated by a `CF_grammar` satisfies the default
    unrestricted characterization. -/
public theorem is_CF_via_cfg_implies_is_CF {L : Language T} (h : is_CF_via_cfg L) :
    is_CF L := by
  obtain ⟨g, rfl⟩ := h
...
-/
public lemma cfg_of_grammar_transforms_iff (g : grammar T) (hg : grammar_context_free g)
    (w₁ w₂ : List (symbol T g.nt)) :
    CF_transforms (cfg_of_grammar g hg) w₁ w₂ ↔ grammar_transforms g w₁ w₂ := by
  constructor;
  · rintro ⟨ r, u, v, hr, rfl, rfl ⟩;
    change r ∈ List.map cfrule_of_grule g.rules at hr
    rcases List.mem_map.mp hr with ⟨a, ha, har⟩
    clear hr
    subst r
  ...
  · rintro ⟨ r, hr, u, v, rfl, rfl ⟩;
    ...
    refine ⟨u ++ r.input_L, r.input_R ++ v,
      List.mem_map.mpr ⟨r, hr, rfl⟩, ?_, ?_⟩
    · rcases hg r hr with ⟨hL, hR⟩
      simp only [hL, hR, List.append_nil, List.nil_append]
      rfl
    · rcases hg r hr with ⟨hL, hR⟩
      simp only [hL, hR, List.append_nil, List.nil_append]
      rfl
...
public lemma cfg_of_grammar_derives_iff (g : grammar T) (hg : grammar_context_free g)
    (w₁ w₂ : List (symbol T g.nt)) :
...
public theorem cfg_of_grammar_language_eq (g : grammar T) (hg : grammar_context_free g) :
...
/-- Every language satisfying the default unrestricted characterization is generated by
    a `CF_grammar`. -/
public theorem is_CF_implies_is_CF_via_cfg {L : Language T} (h : is_CF L) :
    is_CF_via_cfg L := by
  obtain ⟨g, hg, rfl⟩ := h
