...
parser theory.  The canonical language predicate `is_LRk` is defined directly
through the repository's unrestricted (Type-0) grammars.  It uses the direct
handle predicate `grammar.IsLRk` together with the context-free restriction
`grammar_lrk`.  The parallel `CF_grammar.IsLRk` predicate is retained for the
concrete parser development and backwards compatibility.

In either presentation, after a rightmost derivation step the reducible handle
is uniquely determined by the already-built sentential prefix and by `k`
terminal lookahead symbols.  For unrestricted rules the complete
`input_L A input_R` redex is explicit; `grammar_lrk` additionally requires those
contexts to be empty.
...
This is the grammar-side class matching DPDAs.  The direct and compatibility
presentations are related in
`Langlib.Grammars.LR.UnrestrictedCharacterization`, and the automata equivalence
is proved by the two standard constructions in
...
The entire left-hand redex is matched, and everything strictly to its right is
terminal.  For a context-free rule the two context fields are empty, recovering
the usual rightmost context-free rewrite. -/
@[expose]
public def grammar.RewritesRightmost {N : Type} (r : grule T N)
    (u v : List (symbol T N)) : Prop :=
  ∃ (p : List (symbol T N)) (lookahead : List T),
...
@[expose]
public def augmentRule {N : Type} (r : grule T N) : grule T (Option N) where
...
/-- The distinguished production of the augmented unrestricted grammar. -/
@[expose]
public def augmentStartRule : grule T (Option g.nt) where
  input_L := []
  input_N := none
  input_R := []
...
@[expose]
public def augment : grammar T where
  nt := Option g.nt
  initial := none
  rules := augmentStartRule g :: g.rules.map augmentRule

/-- Knuth's semantic handle-uniqueness condition directly on unrestricted
grammar rules, before fresh-start augmentation. -/
@[expose]
public def CoreIsLRk (k : ℕ) : Prop :=
  ∀ (r₁ r₂ : grule T g.nt), r₁ ∈ g.rules → r₂ ∈ g.rules →
    ∀ (p₁ p₂ : List (symbol T g.nt)) (s₁ s₂ y : List T),
      g.DerivesRightmost [symbol.nonterminal g.initial]
        (p₁ ++ r₁.input_L ++ [symbol.nonterminal r₁.input_N] ++ r₁.input_R ++
          s₁.map symbol.terminal) →
      g.DerivesRightmost [symbol.nonterminal g.initial]
        (p₂ ++ r₂.input_L ++ [symbol.nonterminal r₂.input_N] ++ r₂.input_R ++
          s₂.map symbol.terminal) →
      p₂ ++ r₂.output_string ++ s₂.map symbol.terminal =
        p₁ ++ r₁.output_string ++ y.map symbol.terminal →
      lrLookahead k s₁ = lrLookahead k y →
      p₁ = p₂ ∧ r₁ = r₂

/-- Semantic LR(k) handle uniqueness for an unrestricted grammar, checked
after adjoining a genuinely fresh start symbol. -/
@[expose]
public def IsLRk (k : ℕ) : Prop :=
  g.augment.CoreIsLRk k

/-- Core LR lookahead is monotone on unrestricted grammars. -/
public theorem CoreIsLRk.mono {k l : ℕ} (hkl : k ≤ l)
    (hg : g.CoreIsLRk k) : g.CoreIsLRk l := by
  intro r₁ r₂ hr₁ hr₂ p₁ p₂ s₁ s₂ y hd₁ hd₂ hform hlook
  exact hg r₁ r₂ hr₁ hr₂ p₁ p₂ s₁ s₂ y hd₁ hd₂ hform <| by
...
/-- LR lookahead is monotone on unrestricted grammars. -/
public theorem IsLRk.mono {k l : ℕ} (hkl : k ≤ l)
    (hg : g.IsLRk k) : g.IsLRk l :=
  CoreIsLRk.mono g.augment hkl hg

end grammar

/-- An unrestricted grammar is LR(k) when it is context-free and its
fresh-start augmentation satisfies semantic handle uniqueness. -/
@[expose]
public def grammar_lrk (k : ℕ) (g : grammar T) : Prop :=
  grammar_context_free g ∧ g.IsLRk k

/-- Compatibility presentation of LR(k) languages through `CF_grammar`. -/
@[expose]
public def is_LRk_via_cfg (k : ℕ) (L : Language T) : Prop :=
  ∃ g : CF_grammar T, g.IsLRk k ∧ CF_language g = L

/-- The canonical LR(k) language predicate, defined directly through
unrestricted grammars. -/
@[expose]
public def is_LRk (k : ℕ) (L : Language T) : Prop :=
  ∃ g : grammar T, grammar_lrk k g ∧ grammar_language g = L
...
/-- The compatibility `CF_grammar` presentation is monotone in lookahead. -/
public theorem is_LRk_via_cfg_mono {k l : ℕ} (hkl : k ≤ l) {L : Language T}
    (h : is_LRk_via_cfg k L) : is_LRk_via_cfg l L := by
...
public theorem is_LRk_mono {k l : ℕ} (hkl : k ≤ l) {L : Language T}
  ...
  rcases h with ⟨g, ⟨hcf, hg⟩, hL⟩
  exact ⟨g, ⟨hcf, grammar.IsLRk.mono g hkl hg⟩, hL⟩
