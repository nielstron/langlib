...
public def paddedTerminalWord (w : List T) (k : ℕ) : List (Option T) :=
...
reductions. -/
public theorem erasingImageGrammar_generates_padded [Fintype T] (g : grammar T)
    {w : List T} (hw : grammar_generates g w) :
    ∃ k : ℕ, grammar_generates (erasingImageGrammar g) (paddedTerminalWord w k) := by
  rw [erasingImageGrammar]
  apply finite_generates_padded
  change w ∈ grammar_language (restrictGrammar g)
...
the source grammar. -/
public theorem erasingImageGrammar_generates_project [Fintype T] (g : grammar T)
    {w : List (Option T)} (hw : grammar_generates (erasingImageGrammar g) w) :
    grammar_generates g (w.flatMap erasePadding) := by
  have h := finite_generates_project (restrictGrammar g) hw
  change w.flatMap erasePadding ∈ grammar_language g
  rw [grammar_language_eq_restrictGrammar]
  exact h
