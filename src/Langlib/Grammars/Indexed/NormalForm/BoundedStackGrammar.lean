...
/-- Generated-word exact-length low-window branch.

For every generated target, choose the standard shortest accepting trace with least stack
bound among traces of that length. If that trace is `P`-bounded before the late `C`-window
and throughout the late window, the target is accepted in the exact-length bounded flat-path
slice with width `P + 2`. -/
theorem exists_boundedFlatPathLanguage_length_of_late_window_low_branch_isNormalForm
    {g : IndexedGrammar T} [Fintype g.flag] [DecidableEq g.nt] (hNF : g.IsNormalForm)
    (P C : ℕ) :
    ∀ target : List T,
      g.Generates target →
  ...
  obtain ⟨n, B, trace, htrace, hlen, hhead, hlast, hder, hminLength, hbound,
    ...
    exists_shortest_stackBound_minimal_accepting_derivationTrace_of_generates
      (g := g) hgen
  refine ⟨n, B, trace, htrace, hlen, hhead, ?_, ?_, ?_, hbound, ?_, ?_⟩
  · simpa using hlast
  · simpa using hder
  · intro m hm
    exact hminLength m (by simpa using hm)
  · intro C' hC'
    exact hminBound C' (by
      rcases hC' with ⟨trace', htrace', hlen', hhead', hlast', hbound'⟩
      exact ⟨trace', htrace', hlen', hhead', by simpa using hlast', hbound'⟩)
  · intro hbeforeBound hwindowBound
    exact
      boundedFlatPathLanguage_length_of_late_window_trace_bound_isNormalForm
        (g := g) hNF (P := P) (C := C) (n := n) (target := target)
        (trace := trace) htrace hlen hhead (by simpa using hlast)
        hbeforeBound hwindowBound
...
/-- Flat-language form of the saturated combined branch-frontier bridge.

This keeps the saturation data (`Kpop` and the enlarged `Csat` frontier) from
`exists_bound_boundedStackGrammar_generates_of_late_window_certificate_prefix_preserving_frontier_suffix_take_eq_branch_descent_rank_saturated_budget`,
but compiles the conclusion into the bounded flat-path language used by the finite-tape
simulation target. -/
theorem
    exists_bound_boundedFlatPathLanguage_of_late_window_certificate_prefix_preserving_frontier_suffix_take_eq_branch_descent_rank_saturated_budget
    {g : IndexedGrammar T} [Fintype T] [Fintype g.nt] [Fintype g.flag]
    [DecidableEq g.nt] (hNF : g.IsNormalForm) (P L : ℕ) :
    let C :=
      (Set.Finite.toFinset
...
/-- Fixed-prefix-budget form of the saturated flat late-window bridge.

The previous theorem leaves `Bpre` external together with the arithmetic lower bounds needed
by the shrinker. This endpoint chooses the canonical budget `P + C + K + 1` internally and
then enlarges the resulting flat bound by monotonicity. The remaining premise is only the
structural suffix-prefix certificate at that fixed budget. -/
theorem
    exists_bound_boundedFlatPathLanguage_of_late_window_certificate_prefix_preserving_frontier_suffix_take_eq_branch_descent_rank_saturated_budget_fixed_prefixBound
    {g : IndexedGrammar T} [Fintype T] [Fintype g.nt] [Fintype g.flag]
    [DecidableEq g.nt] (hNF : g.IsNormalForm) (P L : ℕ) :
    let C :=
      (Set.Finite.toFinset
                        ...
                        (((surfaceOfTruncatedForm P (u ++ [ISym.indexed A ζ] ++ v),
                          ...
                          ({x : (SurfaceForm g P ×
                            ...
                            Set ((SurfaceForm g P ×
                              ((g.nt × List g.flag) × List T)) × ℕ))) →
                  τ.take ((P + C + K + 1) - P) =
                    (η.drop P).take ((P + C + K + 1) - P)) →
            target ∈ boundedFlatPathLanguage g (L * ((P + C + K + 1 + C) + 2))) := by
  classical
  dsimp only
  let C :=
    (Set.Finite.toFinset
  ...
  refine ⟨n, B, trace, htrace, hlen, hhead, hlast, hder, hminLength, hbound,
    hminBound, ?_⟩
  intro hbeforeBound hsuffixPrefix
  let Bpre := P + C + K + 1
  have hraw :
      target ∈ boundedFlatPathLanguage g
        (L * (max (P + C) (Bpre + C) + 2)) :=
    hflat Bpre hbeforeBound
      (by dsimp [Bpre]; omega)
      (by dsimp [Bpre]; omega)
      (by
        intro i hi hlow hup hhigh A η τ ζ u v q m w n' hmem hηmax hctx
          hwt hwlen hq hm hmSuffix hn' hτsub hτlen hζeq hζsub hζlen hζtake
          hζder hcert hreplacement hτmin htargetSurface hboundedSurface htargetItem
          hlengthItem hdescent
        simpa [Bpre, C] using
          hsuffixPrefix i hi hlow hup hhigh A η τ ζ u v q m w n'
            hmem hηmax hctx hwt hwlen hq hm hmSuffix hn' hτsub hτlen hζeq hζsub
            hζlen hζtake hζder hcert hreplacement hτmin htargetSurface hboundedSurface
...
/-- Low-or-high fixed-prefix form of the saturated flat late-window bridge.

This packages the two branches needed by the remaining minimal-stack argument. Once the
pre-window prefix is `P`-bounded, either the whole late window is already `P`-bounded, in
which case the target is compiled directly by
`boundedFlatPathLanguage_of_late_window_trace_bound_isNormalForm`, or the high-stack branch
supplies the fixed-prefix suffix certificate consumed by
`exists_bound_boundedFlatPathLanguage_of_late_window_certificate_prefix_preserving_frontier_suffix_take_eq_branch_descent_rank_saturated_budget_fixed_prefixBound`.
-/
theorem
    exists_bound_boundedFlatPathLanguage_of_late_window_low_or_suffix_take_eq_branch_descent_rank_saturated_budget_fixed_prefixBound
    {g : IndexedGrammar T} [Fintype T] [Fintype g.nt] [Fintype g.flag]
    [DecidableEq g.nt] (hNF : g.IsNormalForm) (P L : ℕ) :
    let C :=
      (Set.Finite.toFinset
        ...
        ∃ n B : ℕ, ∃ trace : List (List g.ISym),
          IsDerivationTrace g trace ∧
            ...
            ((∀ k (hk : k < trace.length),
              k < trace.length - 1 - C →
                sententialMaxStackHeight (trace.get ⟨k, hk⟩) ≤ P) →
              LowWindow ∨ HighSuffix →
              target ∈
                boundedFlatPathLanguage g (L * ((P + C + K + 1 + C) + 2)))) := by
  classical
  dsimp only
  let C :=
    (Set.Finite.toFinset
  ...
  refine ⟨n, B, trace, htrace, hlen, hhead, hlast, hder, hminLength, hbound,
    hminBound, ?_⟩
  dsimp only
  intro hbeforeBound hcase
  rcases hcase with hlow | hhigh
  · have hlowFlat :
        target ∈ boundedFlatPathLanguage g (L * (P + 2)) :=
      boundedFlatPathLanguage_of_late_window_trace_bound_isNormalForm
        (g := g) hNF (P := P) (C := C) (L := L) (n := n)
        (target := target) (trace := trace) htargetLen htrace hlen hhead hlast
        hbeforeBound hlow
    exact
      boundedFlatPathLanguage_mono
        (g := g) (B := L * (P + 2))
        ...
        (by
          ...
          omega)
        target hlowFlat
  · exact hfixed hbeforeBound hhigh
