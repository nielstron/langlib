...
public inductive LG_rule (T : Type) (N : Type) where
...
public structure LG_grammar (T : Type) where
...
public def LG_rule.lhs {N : Type} : LG_rule T N → N
  | .cons A _ _  => A
  | .single A _  => A
  | .epsilon A   => A
...
- `A → a` outputs `[a]`
- `A → ε` outputs `[]` -/
public def LG_rule.output {N : Type} : LG_rule T N → List (symbol T N)
...
public lemma LG_deri_self (g : LG_grammar T) {w : List (symbol T g.nt)} :
    LG_derives g w w :=
  ReflTransGen.refl
...
public lemma LG_deri_of_deri_tran {g : LG_grammar T} {u v w : List (symbol T g.nt)}
