...
public theorem is_Recursive_univ : is_Recursive (Set.univ : Language T) := by
  let M : TM0.Machine (Option T) Unit := fun _ _ => none
  refine ⟨Unit, inferInstance, inferInstance, M, (fun _ => true), ?_, ?_⟩
  · intro w
    rw [Part.dom_iff_mem]
    refine ⟨TM0.init (w.map Option.some), ?_⟩
    rw [Turing.mem_eval]
...
public theorem is_Recursive_empty : is_Recursive (∅ : Set (List T)) := by
  let M : TM0.Machine (Option T) Unit := fun _ _ => none
  refine ⟨Unit, inferInstance, inferInstance, M, (fun _ => false), ?_, ?_⟩
  · intro w
    rw [Part.dom_iff_mem]
    refine ⟨TM0.init (w.map Option.some), ?_⟩
    rw [Turing.mem_eval]
