...
public def linear_output {N : Type} (s : List (symbol T N)) : Prop :=
...
public def grammar_linear (g : grammar T) : Prop :=
  ∀ r ∈ g.rules,
...
public def is_Linear (L : Language T) : Prop :=
  ∃ g : grammar T, grammar_linear g ∧ grammar_language g = L

/-- The class of linear languages. -/
public def Linear : Set (Language T) :=
  Set.ofPred is_Linear
