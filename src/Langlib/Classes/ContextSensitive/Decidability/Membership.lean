...
Membership in a context-sensitive language is **uniformly computable**: there is a single
algorithm that, given an *encoded* context-sensitive grammar and a word, decides membership.
...
- `contextSensitive_computableMembership` — **uniform** computability: `ComputableMembership`
  for the encoded presentation `contextSensitiveLanguageOf`.
- `contextSensitiveLanguageOf_computableMembership` / `computablePred_mem_of_is_CS` — the weaker
  **per-language** computability (`ComputablePred`), derived from the uniform result.
...
public theorem memCode_primrec [DecidableEq T] [Primcodable T] :
...
public theorem memCode_sound [DecidableEq T] (c : Code T) (v : List T)
...
the search bound. -/
public theorem memCode_complete [DecidableEq T] (c : Code T) (v : List T)
...
public theorem code_of_CS [DecidableEq T] {L : Language T} (hL : is_CS L) :
...
public theorem contextSensitiveLanguageOf_computableMembership [DecidableEq T] [Primcodable T]
    (c : Code T) : ComputablePred (fun w : List T => w ∈ contextSensitiveLanguageOf c) := by
  show ComputablePred (fun w : List T => memCode c w = true)
  exact ComputablePred.computable_iff.mpr
    ⟨fun w => memCode c w,
      (memCode_primrec.comp (Primrec.pair (Primrec.const c) Primrec.id)).to_comp, rfl⟩

/-- Every context-sensitive language is exactly the bounded-search language of some code. -/
public theorem exists_code_of_is_CS [DecidableEq T] {L : Language T} (hL : is_CS L) :
    ∃ c : Code T, contextSensitiveLanguageOf c = L := by
...
public theorem computablePred_mem_of_is_CS [DecidableEq T] [Primcodable T] {L : Language T}
    (hL : is_CS L) : ComputablePred (fun w : List T => w ∈ L) := by
  obtain ⟨c, hc⟩ := exists_code_of_is_CS hL
  rw [← hc]
  exact contextSensitiveLanguageOf_computableMembership c

/-! ## Decidable membership

Decidability is a corollary of computability. For a coded grammar the decision procedure is the
genuine bounded search; for an abstract context-sensitive language it is extracted classically. -/

/-- Membership in a coded grammar's bounded-search language is decidable (by the search). -/
public instance decidable_mem_contextSensitiveLanguageOf [DecidableEq T] (c : Code T) (v : List T) :
    Decidable (v ∈ contextSensitiveLanguageOf c) :=
  decidable_of_iff (memCode c v = true) Iff.rfl

/-- Membership in a context-sensitive language is decidable. -/
public noncomputable def decidable_mem_of_is_CS [DecidableEq T] [Primcodable T] {L : Language T}
    (hL : is_CS L) (v : List T) : Decidable (v ∈ L) :=
  (exists_code_of_is_CS hL).choose_spec ▸
    decidable_mem_contextSensitiveLanguageOf (exists_code_of_is_CS hL).choose v
