...
/-- The unrestricted grammar obtained from a `CS_grammar` is non-contracting,
and hence context-sensitive in the `S → ε`-optional sense. -/
public theorem grammar_of_csg_context_sensitive (g : CS_grammar T) :
  ...
  intro r hr
  change grule T g.nt at r
  change r ∈ List.map _ g.rules at hr
  obtain ⟨cr, hcr, rfl⟩ := List.mem_map.mp hr
  have hpos : 0 < cr.output_string.length :=
    List.length_pos_of_ne_nil (g.output_nonempty cr hcr)
  change cr.context_left.length + 1 + cr.context_right.length ≤
    (cr.context_left ++ cr.output_string ++ cr.context_right).length
  simp only [List.length_append]
  omega

/-- Every language generated by an ε-free context-preserving grammar is
context-sensitive. -/
