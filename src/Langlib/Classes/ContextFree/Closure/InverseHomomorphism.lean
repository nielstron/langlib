...
lemma is_CF_sLang [Fintype α] : is_CF (sLang α β) := by
  have h_cfl_union : ∀ s : Finset α,
      is_CF (⋃ a ∈ s, {[Sum.inl a]} : Language (α ⊕ β)) := by
    intro s
    ...
    | empty =>
        ...
        · contradiction
    | @insert a s ha ih =>
        let tail : Language (α ⊕ β) := ⋃ x ∈ s, ({[Sum.inl x]} : Language (α ⊕ β))
        have heq : (⋃ x ∈ insert a s, {[Sum.inl x]} : Language (α ⊕ β)) =
            ({[Sum.inl a]} : Language (α ⊕ β)) + tail := by
          change (⋃ x ∈ insert a s, ({[Sum.inl x]} : Set (List (α ⊕ β)))) =
            ({[Sum.inl a]} : Set (List (α ⊕ β))) ∪
              (⋃ x ∈ s, ({[Sum.inl x]} : Set (List (α ⊕ β))))
          ext w
          simp
        rw [heq]
        rw [is_CF_iff_isContextFree]
        apply Language.IsContextFree.add
        · exact isContextFree_singleton [Sum.inl a]
        · have ihtail : is_CF tail := by
            unfold tail
            exact ih
          exact (is_CF_iff_isContextFree).mp ihtail
  have heq : sLang α β =
...
lemma flatMap_fHom_kstar_sLang (v : List (α ⊕ β)) (hv : v ∈ KStar.kstar (sLang α β)) :
  ...
  obtain ⟨L, rfl, hL⟩ := hv
  apply flatMap_fHom_allInl
  intro x hx
  rw [List.mem_flatten] at hx
  obtain ⟨u, huL, hxu⟩ := hx
  obtain ⟨a, rfl⟩ := hL u huL
  simp only [List.mem_singleton] at hxu
  subst x
  rfl
...
lemma fInv_eq (L : Language β) :
    ...
    (L.subst (substFn α β)) * KStar.kstar (sLang α β) := by
      ...
      · obtain ⟨ v, hv, w, hw, rfl ⟩ := hu;
        ...
        change List.flatMap fHom (v ++ w) ∈ L
        rw [List.flatMap_append, h_flatMap, h_flatMap_w, List.append_nil]
        exact hw'
      ...
      have h_ind : ∀ (k : ℕ) (hk : k ≤ (h a).length), List.foldl (dStep h) (if hl : 0 < (h a).length then if hl : k < (h a).length then some (some ⟨a, ⟨k, hl⟩⟩) else some none else some none) (List.map Sum.inr (List.drop k (h a))) = some none := by
        ...
        · rcases eq_or_lt_of_le hk <;> simp_all +decide [  ];
          ...
          · split_ifs <;> simp_all +decide [ dStep ];
            grind;
          · rw [List.drop_eq_getElem_cons (by simp_all)]
            rw [List.get_eq_getElem, List.getElem_map]
      have hind := h_ind 0 (Nat.zero_le _)
      by_cases hl : 0 < (h a).length
      · rw [dif_pos hl, dif_pos hl] at hind
        simpa [embedWord, dStep, hl] using hind
      · rw [dif_neg hl] at hind
        simpa [embedWord, dStep, hl] using hind
...
lemma dfa_accepts_of_dLang (h : α → List β) (v : List (α ⊕ β)) (hv : v ∈ dLang h) :
    v.foldl (dStep h) (some none) = some none := by
      change ∃ ws : List (List (α ⊕ β)), v = ws.flatten ∧
        ∀ w ∈ ws, ∃ a : α, w = embedWord h a at hv
      obtain ⟨ws, hv_eq, hws⟩ := hv
    ...
    ((fInv L ⊓ dLang h) : Language (α ⊕ β)).homomorphicImage piHom := by
      ...
      · intro hw
        ...
        · have h_flatMap : (extendHom (embedWord h) w).flatMap piHom = w := by
          ...
          rw [Language.mem_list_prod_iff_forall2]
          refine ⟨List.map piHom (extendHom (embedWord h) w), ?_, ?_⟩
          · simpa only [List.flatMap_def] using h_flatMap.symm
          · induction extendHom (embedWord h) w with
            | nil => exact List.Forall₂.nil
            | cons x xs ih =>
                simp only [List.map_cons]
                exact List.Forall₂.cons (Set.mem_singleton _) ih
      · rintro ⟨ v, hv₁, hv₂ ⟩;
