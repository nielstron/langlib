...
Proof idea: thread DFA states through the nonterminals of the context-free
grammar. A product nonterminal records both the original grammar symbol and the
DFA state change that the substring generated by that symbol must realize, so a
complete derivation exists exactly when the CFG derives the word and the DFA
accepts it.
