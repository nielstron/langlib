...
/-- Bounded concrete rule-path finite-ball form of the current simulator target.

On each fixed input-length ball, one packed width works for every target in the ball, and
membership is exactly a concrete reverse rule path whose length is bounded by the finite
packed-row state space. -/
public theorem exists_terminalRow_rulePath_card_bound_eq_on_length_le_of_isNormalForm
    {A : Type} [Fintype A]
    (g : IndexedGrammar A) [Fintype g.nt] [Fintype g.flag] [DecidableEq g.nt]
    (hNF : g.IsNormalForm) (L : ℕ) :
    ∃ B : ℕ, ∀ target : List A,
      ...
      (target ∈ g.Language ↔
        ...
        ∃ path : List (IndexedGrammar.PackedFlatForm g (B + 2) target.length),
          ...
          path.IsChain
            (fun x y =>
              IndexedGrammar.PackedFlatRuleStep g (B + 2) target.length y x)) :=
...
/-- Bounded concrete rule-path form of the fixed-width reverse row language. -/
public theorem packedReverseRuleStepRowLanguage_iff_exists_row_rulePath_card_bound
    {A : Type} [Fintype A]
    ...
    cells ∈ IndexedGrammar.packedReverseRuleStepRowLanguage g B ↔
      ...
      ∃ path : List
        ...
        path.IsChain
          (fun x y =>
            IndexedGrammar.PackedFlatRuleStep g (B + 2) cells.length y x) :=
...
/-- Flat-list edge-witness form of the fixed-width terminal target. -/
public theorem
    packedTerminalReverseRuleStep_language_iff_exists_terminalRow_flatRuleStep_path_card_bound
    {A : Type} [Fintype A]
    (g : IndexedGrammar A) [Fintype g.nt] [Fintype g.flag]
    (B : ℕ) {w : List A} :
    w ∈ IndexedGrammar.packedTerminalReverseRuleStepLanguage g B ↔
      ...
      ∃ path : List (IndexedGrammar.PackedFlatForm g (B + 2) w.length),
        ...
        path.IsChain (fun x y =>
          ∃ x₀ y₀ : List (IndexedGrammar.FlatSymbol A g.nt g.flag),
            ...
            x =
              IndexedGrammar.packedFlatForm g (B + 2) w.length x₀ ∧
            y =
              IndexedGrammar.packedFlatForm g (B + 2) w.length y₀ ∧
            IndexedGrammar.FlatRuleStep g y₀ x₀) :=
  IndexedGrammar.packedTerminalReverseRuleStepLanguage_iff_exists_terminalRow_flatRuleStep_path_card_bound
    (g := g) B

/-- Flat-list edge-witness target for the finite normal-form core. -/
public theorem finite_normalForm_language_iff_exists_terminalRow_flatRuleStep_path_card_bound
    {A : Type} [Fintype A] [DecidableEq A]
    (g : IndexedGrammar A) [Fintype g.nt] [Fintype g.flag] [DecidableEq g.nt]
    (hNF : g.IsNormalForm) {w : List A} :
    w ∈ g.Language ↔
      ...
      ∃ path : List (IndexedGrammar.PackedFlatForm g (B + 2) w.length),
        ...
        path.IsChain (fun x y =>
          ∃ x₀ y₀ : List (IndexedGrammar.FlatSymbol A g.nt g.flag),
            ...
            y =
              IndexedGrammar.packedFlatForm g (B + 2) w.length y₀ ∧
            IndexedGrammar.FlatRuleStep g y₀ x₀) := by
  rw [finite_normalForm_language_iff_exists_packedTerminalReverseRuleStepLanguage
    (g := g) hNF]
  constructor
  · rintro ⟨B, hB⟩
