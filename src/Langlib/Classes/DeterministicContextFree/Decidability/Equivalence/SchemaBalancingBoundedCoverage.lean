...
/-!
# Bounded fixed-argument balancing coverage

This strengthens the fixed-argument Proposition-45 assembly with the uniform
presentation bounds from `SchemaBalancingSizeBounds`.  The output keeps the
supplied argument tuple verbatim.
-/
...
/-- The bounded balancing-result coverage together with the exact schema pair
used by its construction.  Retaining this equality lets marker-stable
consumers prove original-rank well-formedness without reopening the complete
certificate assembly. -/
public structure StructuredBoundedWitnessedBalancingResult
  ...
  coverage : BoundedWitnessedActiveSchemaCoverage
    g initialLeft initialRight
    (g.fixedBalancingPresentationBound bound arity schemaBound)
    width arguments
    (stem ++ activeResult.actions.map Sum.inl)
  schema_eq :
    coverage.witnessed.coverage.schema =
      (arity, family.composedActiveSchema activeResult,
        pivotResult.residual)

/-- Assemble a complete bounded balancing row while retaining the supplied
argument tuple verbatim and recording its exact schema pair. -/
public theorem exists_structuredBoundedWitnessedBalancingResult
    (family : segment.SchemaExposedSuccessorFamily
      (initialLeft := initialLeft) (initialRight := initialRight)
      stem children pivotSchema arguments arity width)
    (hg : g.WellFormed)
    ...
    Nonempty (StructuredBoundedWitnessedBalancingResult
      family activeResult pivotResult schemaBound) := by
  classical
  let activeArguments :=
  ...
  let witnessed : WitnessedActiveSchemaCoverage
      g initialLeft initialRight width
      (stem ++ activeResult.actions.map Sum.inl) :=
    { coverage := coverage
      left_witness := by
  ...
  let bounded : BoundedWitnessedActiveSchemaCoverage
      g initialLeft initialRight
      (g.fixedBalancingPresentationBound bound arity schemaBound)
      width arguments
      (stem ++ activeResult.actions.map Sum.inl) :=
    { witnessed := witnessed
      arguments_eq := by
        rfl
      arity_le := by
        simpa [witnessed, coverage, schema, BasisPair.arity] using
          g.le_fixedBalancingPresentationBound_arity
            bound arity schemaBound
      left_size_le := by
        simpa [witnessed, coverage, schema, BasisPair.left,
          leftSchema] using
            family.composedActiveSchema_nodes_length_le_fixedBalancingPresentationBound
              hactive hroot activeResult hpivotSchemaSize
      right_size_le := by
        simpa [witnessed, coverage, schema, BasisPair.right,
          rightSchema] using
            pivotResult.nodes_length_le_fixedBalancingPresentationBound
              hpivotSchemaSize }
  exact ⟨
    { coverage := bounded
      schema_eq := by
...
/-- Compatibility projection of the structured bounded result. -/
public theorem exists_boundedWitnessedBalancingResult
    (family : segment.SchemaExposedSuccessorFamily
      (initialLeft := initialLeft) (initialRight := initialRight)
      stem children pivotSchema arguments arity width)
    (hg : g.WellFormed)
    ...
    {schemaBound : ℕ}
    (hpivotSchemaSize : pivotSchema.nodes.length ≤ schemaBound) :
    Nonempty (BoundedWitnessedActiveSchemaCoverage
      g initialLeft initialRight
      (g.fixedBalancingPresentationBound bound arity schemaBound)
      width arguments
      (stem ++ activeResult.actions.map Sum.inl)) := by
  obtain ⟨result⟩ :=
    family.exists_structuredBoundedWitnessedBalancingResult
      hg hbound hactive hpivot hfiller hroot houter hequivalent
...
/-- Full bounded fixed-argument Proposition 45 for one normal-form balancing
segment.  The supplied argument tuple and path word are preserved exactly. -/
public theorem BalancingSegment.exists_schemaBoundedWitnessedBalancingResult
  ...
  obtain ⟨coverage⟩ :=
    family.exists_boundedWitnessedBalancingResult hg hbound
