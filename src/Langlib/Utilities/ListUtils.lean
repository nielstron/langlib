...
public lemma reverse_append_append :
...
public lemma forall_mem_append_append {p : α → Prop} :
...
@[expose]
public def n_times (l : List α) (n : ℕ) : List α :=
...
@[expose]
public def count_in (l : List α) (a : α) : ℕ :=
List.sum (List.map (λ s => ite (s = a) 1 0) l)

public lemma count_in_nil (a : α) :
  count_in [] a = 0 := rfl

public lemma count_in_cons (a b : α) :
  count_in (b :: x) a  =  ite (b = a) 1 0  +  count_in x a  := by
  unfold count_in
  simp

public lemma count_in_append (a : α) :
  count_in (x ++ y) a  =  count_in x a  +  count_in y a  := by
  unfold count_in
  simp

public lemma count_in_replicate_eq (a : α) (n : ℕ) :
...
public lemma count_in_replicate_neq {a b : α} (hyp : a ≠ b) (n : ℕ) :
...
public lemma count_in_singleton_eq (a : α) :
  count_in [a] a  =  1  := List.count_in_replicate_eq a 1

public lemma count_in_singleton_neq {a b : α} (hyp : a ≠ b) :
  count_in [a] b  =  0  := List.count_in_replicate_neq hyp 1

public lemma count_in_pos_of_in {a : α} (hyp : a ∈ x) :
  count_in x a > 0 := by
  induction x with
  | nil => simp at hyp
  | cons d l ih =>
...
public lemma count_in_zero_of_notin {a : α} (hyp : a ∉ x) :
  count_in x a = 0 := by
  induction x with
  | nil => rfl
  | cons d l ih =>
...
public lemma count_in_flatten (L : List (List α)) (a : α) :
...
public lemma split_at_separator {α : Type} {a b u mid v : List α} {sep : α}
...
public lemma head_unique_elem {α : Type} {s : List α} {elem : α} {u rest : List α}
...
@[expose]
public def nth (l : List α) (n : Nat) : Option α :=
  match l, n with
  | [], _ => none
  | a :: _, 0 => some a
...
@[expose]
public def nthLe : (l : List α) → (n : Nat) → n < l.length → α
| [], _, h => (Nat.not_lt_zero _ h).elim
| a :: _, 0, _ => a
| _ :: tail, Nat.succ n, h =>
    nthLe tail n (by simpa using (Nat.lt_of_succ_lt_succ h))

public theorem get?_eq_nth {l : List α} {n : Nat} : l[n]? = l.nth n := by
...
public lemma nthLe_mem {l : List α} {n : ℕ} (h : n < l.length) :
  l.nthLe n h ∈ l := by
  induction l generalizing n with
  | nil => exact (Nat.not_lt_zero _ h).elim
  | cons a l ih =>
...
public theorem nthLe_map {f : α → β} {l : List α} {n : Nat}
...
public theorem nthLe_append_right {l₁ l₂ : List α} {n : Nat}
...
public theorem nthLe_congr {l : List α} {n : Nat} {h₁ h₂ : n < l.length} :
    l.nthLe n h₁ = l.nthLe n h₂ := by
  congr 1

public theorem nthLe_of_eq {l₁ l₂ : List α} (h : l₁ = l₂) {n : Nat} (hn : n < l₁.length) :
    l₁.nthLe n hn = l₂.nthLe n (by simpa [h] using hn) := by
  subst h
  rfl

public theorem nthLe_append {l₁ l₂ : List α} {n : Nat}
...
public theorem nthLe_of_mem {l : List α} {a : α} (h : a ∈ l) :
    ∃ n, ∃ h' : n < l.length, l.nthLe n h' = a := by
  exact (mem_iff_nth_le).1 h

public theorem nthLe_replicate (a : α) {n i : Nat} (h : i < (List.replicate n a).length) :
...
public theorem nth_take {l : List α} {m n : Nat} (h : n < m) :
...
public theorem nth_drop {l : List α} {m n : Nat} :
...
public theorem nth_append_right {l₁ l₂ : List α} {n : Nat} (h : l₁.length ≤ n) :
...
public theorem nth_append {l₁ l₂ : List α} {n : Nat} (h : n < l₁.length) :
